...
struct Memory {
    ...
    // it either leads to inconvenient API with function like "void freeBuffer(void* _buffer)" so that memory
    // will be deallocated compatibly with method that was used for its allocation
    // we decided that it is better to make _buffer allocation responsibility of calling code
